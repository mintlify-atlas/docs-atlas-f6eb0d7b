...
description: "NuvioTV offers Essential mode for simplified setup and Advanced mode for full control. Learn what each mode shows, how to switch, and how defaults are applied for new installs."
---
...
<Tabs>
  <Tab title="Essential Mode">
    **Essential mode** is designed for users who want a streamlined path from install to playback. It answers one question: *I just want NuvioTV to work with the least setup possible.*
    ...
    - Basic playback controls: player preference, stream selection (Manual or Auto), auto-play next episode, P2P toggle.
    - Subtitle and audio basics: preferred language and subtitle size.
...
| Player preference | Show | Show | Recovery path if internal playback fails |
...
`AppFeaturePolicy` gates certain features at build time regardless of experience mode:

| Policy Flag | Full Flavor | Play Store Flavor |
|---|---|---|
| `pluginsEnabled` | `true` | `false` |
| `inAppUpdatesEnabled` | `true` | `false` |
| `inAppTrailerPlaybackEnabled` | `true` | `false` |
| `externalTrailerPlaybackEnabled` | `true` | `true` |
| `trailerPlaybackMode` | `IN_APP` | `EXTERNAL` |
| `imdbRatingLogoEnabled` | `true` | `false` |

Even in Advanced mode, plug-ins and in-app updates are not shown in the Play Store flavor because `AppFeaturePolicy.pluginsEnabled` is `false`. Experience mode visibility checks are always combined with build policy checks.

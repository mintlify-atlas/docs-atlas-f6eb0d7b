...
description: "Create up to six user profiles with separate watch history, addon lists, and PIN protection. Sync profiles across devices with a Nuvio account."
---
...
**Secondary profiles** (ids 2–6) have two addon modes, set at creation time via the `usesPrimaryAddons` flag on the `UserProfile` data class (which also carries `usesPrimaryPlugins`, `avatarId`, and `avatarUrl`):
